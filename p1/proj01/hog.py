...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    for i in range(0, num_rolls):
        ...
        if score_each == 1:
            flag = 1
        if current_score % 2 == score_each % 2:
            step += 1
        if flag == 1:
            score = 0
        else:
...
def swine_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    v = 3 ** (player_score + opponent_score)
    if v < 10:
        return True
    i = 1
    first_bit = 0
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce(score0, score1):
        if who == 0:
            pv = score0 - last_score
            if pv > running_high:
                print(pv, 'point(s)!', "That's a record gain for Player 0!")
                return announce_highest(who, score0, pv)
            else:
                return announce_highest(who, score0, running_high)
        elif who == 1:
            pv = score1 - last_score
            if pv > running_high:
                print(pv, 'point(s)!', "That's a record gain for Player 1!")
                return announce_highest(who, score1, pv)
            else:
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    num_roll = 0
    num_s = 0
    for cou in range(1, 11):
        average_dice = make_averaged(roll_dice, trials_count)
        temp_v = average_dice(current_score, cou, dice)
        if num_s < temp_v:
            num_s = temp_v
            num_roll = cou
    return num_roll
...
def swine_swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if swine_swap(score, opponent_score) and opponent_score - score >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
